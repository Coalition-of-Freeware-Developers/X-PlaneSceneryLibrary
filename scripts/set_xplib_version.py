#!/usr/bin/env python3
"""
Update xplib/config/resource.h version macros to match a provided version.

Usage:
  python scripts/set_xplib_version.py 1.2.3[.4]

Updates:
  - XPLANE_LIBRARY_MAJOR, XPLANE_LIBRARY_MINOR, XPLANE_LIBRARY_PATCH, XPLANE_LIBRARY_BUILD
  - XPLANE_LIB_VERSION_STRING
  - XPLANE_LIB_VERSION (hex packed: MAJ<<24 | MIN<<16 | PAT<<8 | BLD)
"""
...
def main(argv: list[str]):
    if len(argv) < 2:
        raise SystemExit("Usage: python scripts/set_xplib_version.py <X.Y.Z[.W]>")
    ...
    res_path = repo_root / "xplib" / "config" / "resource.h"
    ...
    sub_define("XPLANE_LIBRARY_MAJOR", major)
    sub_define("XPLANE_LIBRARY_MINOR", minor)
    sub_define("XPLANE_LIBRARY_PATCH", patch)
    sub_define("XPLANE_LIBRARY_BUILD", build)

    # Update XPLANE_LIB_VERSION_STRING
    content = re.sub(
        r'^(#define\s+XPLANE_LIB_VERSION_STRING\s+\")[^"]*(\")\s*$',
        rf"\g<1>{version_str}\g<2>",
        content,
        flags=re.MULTILINE,
    )

    # Update XPLANE_LIB_VERSION (hex)
    content = re.sub(
        r'^(#define\s+XPLANE_LIB_VERSION\s+)0x[0-9A-Fa-f]+\s*$',
        rf"\g<1>{hex_ver}",
        content,
        flags=re.MULTILINE,
    )
